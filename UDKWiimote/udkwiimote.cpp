...
#pragma region ball_tracking_vars
	CvSize size;
	CvCapture* capture;
...
#pragma endregion Calibration
	...
	void NewBallTracker()
	{
		if (!capture)
		{
			printf( "Ball tracking video feed not initialized properly" );
			return;
		}
		...
		cvErode(hsv_frame, hsv_frame, NULL, 6);
		cvDilate(hsv_frame, hsv_frame, NULL, 6);
	}
	...
	void UpdateBallTracking()
	{
		...
		CvSeq* circles = cvHoughCircles(thresholded, storage, CV_HOUGH_GRADIENT, 2,
			thresholded->height/4, 100, 50, 7, 400);
		...
		if (circles->total >= 1)
		{
			...
			ball.x = (sqrt(p[2])-2)*50.0; //3 - 10
			ball.y = -(p[0]/4.0); //0 - 640
			ball.z = -(p[1]/4.0); //0 - 480
	...
	unsigned __stdcall BallTrackingThread(void* param)
	{
		while(1)
			UpdateBallTracking();
	}

	__declspec(dllexport) FVector wiiMoteBallPosition()
	{
		return ball;
	}
	...
	__declspec(dllexport) bool wiiMoteInitialize()
	{
		...
		//Initialize to realistic values
		ball.x = 320;
		ball.y = 240;
		ball.z = 25;
		...
		trackingThreadHandle = (HANDLE)_beginthreadex(NULL, 0, BallTrackingThread, NULL, 0, NULL);
		SetThreadPriority(trackingThreadHandle, WORKER_THREAD_PRIORITY);
